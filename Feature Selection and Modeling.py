...
# -*- coding: utf-8 -*-
"""
Radiomics ML Batch Runner (Final Script)
=======================================
目标：
1) 用一个“批量实验计划表”（Excel）批量指定每次实验的参数；
2) 自动逐行跑实验，每行一个输出文件夹（001, 002, ... 或使用 job_id）；
3) 每个实验文件夹内：
   - settings.txt：记录本次实验所有参数与环境信息
   - run.log：控制台输出日志
   - task_<task>/：该任务下所有图（ROC、SHAP）与表（label counts、AUC table、summary 等）

新增功能（按你的要求）：
A) Train/Test 按 ID 闭区间划分（4 个参数）：
   - TRAIN_ID_MIN, TRAIN_ID_MAX, TEST_ID_MIN, TEST_ID_MAX
   规则：
   - 四个参数都给齐才启用 split；否则走旧逻辑：全部样本仅做 5-fold OOF
   - 两区间若有重叠，直接报错停止“本轮实验”
   - 若区间越界（超过该 task 可用 ID 的最小/最大），直接报错停止“本轮实验”

B) 特征筛选（Per-Modality 模式，始终执行）：
   - 算法：LASSO / mRMR（Per-Modality 模式）
   - 各模态独立执行：Spearman 去相关 → mRMR → LASSO
   - 筛选在 TRAIN 上执行；筛选后在 log 中记录剩余特征数，并保存最终 feature 列表
   - 训练集 OOF 与测试集推理都只用筛出来的特征

C) SMOTE（SMOTE=True/False）：
   - 若 True：在“特征筛选之后”对每个 task 的训练数据做过采样平衡
   - OOF：每个 fold 的训练子集内做 SMOTE（避免泄漏）
   - Test：fit full-train 时对 train 做一次 SMOTE，再在 test 上推理

D) SHAP：
   - 每个 task：对"每个模型"都输出两套 SHAP（train 上 2 张 + test 上 2 张）
   - SHAP分析始终在SMOTE前的原始训练数据上进行（避免合成样本影响特征重要性估计）
   - 若 test 集为空（未启用 split），则仅输出 train SHAP（与旧逻辑一致）

E) DCA (Decision Curve Analysis) - 多模型合并图：
   - 每个 task：汇总所有算法的DCA结果到同一张图上（类似ROC曲线）
   - 训练集、验证集、外部验证集各生成一张合并DCA图
   - Treat-All净收益限制不为负（标准DCA约定），Y轴范围自动适配
   - 阈值概率范围 0-1.0，与"Treat All"和"Treat None"策略对比
   - 文件名格式：{task}_dca_train_fig001.png / {task}_dca_test_fig001.png / {task}_dca_external_fig001.png

F) Calibration Curve - 多模型合并图：
   - 每个 task：汇总所有算法的校准曲线结果到同一张图上
   - 训练集、验证集、外部验证集各生成一张合并校准曲线图
   - 使用分箱法(quantile strategy)计算校准曲线
   - 图例中显示每个模型的Brier Score
   - 文件名格式：{task}_cal_train_fig001.png / {task}_cal_test_fig001.png / {task}_cal_external_fig001.png

G) PR-AUC (Precision-Recall AUC)：
   - 对每个模型计算PR-AUC，使用bootstrap计算95%置信区间和标准差
   - 训练集(OOF)、验证集、外部验证集均生成PR-AUC结果表
   - 训练集、验证集、外部验证集均生成合并PR曲线图（含基线参考线）
   - 文件名格式：{task}_pr_auc_train_table001.xlsx / {task}_pr_curve_train_fig001.png 等

H) 约登指数 (Youden Index)：
   - 对每个模型计算最优阈值下的约登指数
   - 同时输出该阈值下的灵敏度(Sensitivity)、特异度(Specificity)、PPV、NPV
   - 以及TP、TN、FP、FN混淆矩阵元素
   - 训练集(OOF)、验证集、外部验证集均生成结果表
   - 文件名格式：{task}_youden_train_table001.xlsx 等

重要说明：
- 你现有 notebook 里的“功能函数”（读取/清洗/建模/绘图）我尽量保持原样并继承；
- 本脚本通过 monkey patch 拦截 display() 和 plt.show()：
  display 输出表格会保存，plt.show 输出图会保存。
"""
...
# XGBoost (safe import) - compatible with both 1.x and 2.x
try:
    import xgboost as xgb
    # Try to import XGBClassifier from sklearn module (XGBoost 2.x)
    from xgboost.sklearn import XGBClassifier
    # Disable XGBoost verbose output to prevent binary serialization data in logs
    xgb.set_config(verbosity=0)
    XGBOOST_AVAILABLE = True
except ImportError:
    try:
        # Fallback for XGBoost 1.x
...
# ============================================================
# 0) DEFAULTS (used when a column is missing/blank in plan)
# ============================================================
DEFAULT_CONFIG = {
    # file paths
    "OUTCOME_XLSX": "./Feature Data-v1/Clinical Outcomes-v2.xlsx",
    "OUTCOME_SHEET": "Sheet1",
    "SUV_XLSX": "./Feature Data-v1/SUV-feature-v2.xlsx",
    "T2W_XLSX": "./Feature Data-v1/T2W-feature-v2.xlsx",
    "ADC_XLSX": "./Feature Data-v1/ADC-feature-v2.xlsx",
    ...
    "CLINICAL_BASE_FEATURES": ["Age", "Height", "Weight", "tPSA"],
    "INCLUDE_bSIUP_GG": True,

    # tasks
    "Y_CANDIDATES": ["psISUP GG", "Burden", "Margin", "EPE", "PNI", "SVI", "VDI", "IDC-P", "PDA", "PDA-IDC-P"],
    ...
    # extras
    "RUN_SHAP": True,          # may be slow

    # ===== NEW: Train/Test split by random division =====
    "Internal_MIN": None,      # 样本ID范围最小值
    "Internal_MAX": None,      # 样本ID范围最大值
    "Divide": None,            # 划分比例，如 "5:3"
    "Divide_Random": 42,       # 随机划分种子

    # ===== NEW: External validation =====
    "EXTERNAL_ID_MIN": None,   # 外部独立验证集ID范围最小值
    "EXTERNAL_ID_MAX": None,   # 外部独立验证集ID范围最大值

    # ===== NEW: Feature selection =====
    "FS_TOPK": 30,                 # 最终保留特征数（统一值，不再由训练文件指定）
    "FS_MRMR_TOPK": 25,            # 各模态mRMR保留特征数（统一值，不再由训练文件指定）
    "FS_LASSO_TOPK": 7,            # 各模态LASSO保留特征数（统一值，不再由训练文件指定）
    "FS_CORR_THRESHOLD": 0.8,      # Spearman相关性筛选阈值
...
# ============================================================
# 1) EXISTING FUNCTIONS (KEEP AS-IS)
# ============================================================

def _standardize_null(x):
    """把 'NULL' / '' / nan 统一成 np.nan"""
    if x is None:
        return np.nan
...
def build_y_module(out_df):
    """
    返回：dict[y_name] = Series(index=ID, values in {0,1} or NaN)
    规则：
      - psISUP GG: <=2 vs >2
      - Burden: <=20 vs >20
      - 其他：0/1，NULL 视作缺失
    ...
    # psISUP GG
    if "psISUP GG" in tmp.columns:
        s = tmp["psISUP GG"].map(_to_numeric_or_nan)
        y_dict["psISUP GG"] = (s > 2).astype("float")
    else:
        print("[WARN] Outcomes 无列 psISUP GG")

    # Burden
    if "Burden" in tmp.columns:
        s = tmp["Burden"].map(_to_numeric_or_nan)
        y_dict["Burden"] = (s > 20).astype("float")
    else:
        print("[WARN] Outcomes 无列 Burden")

    # 其他二分类 0/1
    for col in ["Margin", "EPE", "PNI", "SVI", "VDI", "IDC-P", "PDA", "Gene Alternation"]:
        if col in tmp.columns:
            s = tmp[col].map(_to_numeric_or_nan)
            s2 = s.where(s.isin([0.0, 1.0]), np.nan)
            y_dict[col] = s2
        else:
            print(f"[WARN] Outcomes 无列 {col}")

    for k in list(y_dict.keys()):
        y_dict[k] = y_dict[k].astype("float")
    return y_dict

def parse_pyradiomics_sheet(path, sheet_name, image_types=None, drop_diagnostics=True):
    """
    解析你这种导出格式：
      - 第0行（从第4列开始）: Patient ID
      - 第1行（从第4列开始）: Patient Name
      - 第2行: 列0..2 是 meta 标题（不强依赖）
      - 第3行及以后: [Image type, Feature Class, Feature Name] + 患者列数值

    返回：X (patients x features), index=ID
    """
    if not os.path.exists(path):
...
def build_radiomics_module(
    ...
    """
    支持多个 sheet + 多个 image type，合并成一个 df (index=ID)
    特征名会加前缀：{modality_prefix}__{sheet}__{feature_name}
    """
    all_X = []
    for sh in sheets:
...
def build_clinical_feature_module(
    ...
    """
    从 outcomes 里抽取 Age/Height(or Hight)/Weight + 可选 bSIUP GG
    返回 df(index=ID)
    """
...
def build_full_feature_table_custom(
    ...
    """
    wrapper：等价于你原来的 build_full_feature_table，但支持每次实验自定义 CLINICAL_BASE_FEATURES。
    """
    parts = {}
...
class PerDatasetZScoreScaler(BaseEstimator, TransformerMixin):
    """
    对每次传入的数据单独做 Z-Score。
    - fit 不保存均值/方差，避免 train/test 共用同一套统计量
    - transform 时按当前数据集自身统计量标准化
    """
...
def build_models():
    """
    返回 dict[name] = estimator(Pipeline or classifier)
    备注：
      - LinearSVC 无 predict_proba，使用 CalibratedClassifierCV 转成可输出概率
      - 当前模型：LogReg_ElasticNet, Calib_LinearSVC, SVM, RandomForest, XGBoost
      - XGBoost 需要安装 xgboost 包才能使用
    """
    models = {}
...
def add_task_pda_idcp(y_dict: dict):
    """
    增加复合任务 PDA-IDC-P：
      - 正例：PDA==1 或 IDC-P==1
      - 负例：PDA==0 且 IDC-P==0
      - 其余情况 -> NaN（更保守）
    """
    if "PDA-IDC-P" in y_dict:
        return y_dict
    if "PDA" not in y_dict or "IDC-P" not in y_dict:
        return y_dict

    p = y_dict["PDA"]
    i = y_dict["IDC-P"]
    idx = p.index.union(i.index)
    p2 = p.reindex(idx)
    i2 = i.reindex(idx)

    comp = pd.Series(np.nan, index=idx, dtype="float")
    comp[(p2 == 1) | (i2 == 1)] = 1.0
    comp[(p2 == 0) & (i2 == 0)] = 0.0
    y_dict["PDA-IDC-P"] = comp
    return y_dict


# ============================================================
# 2) BATCH-RUNNER UTILITIES (safe to edit)
# ============================================================
...
class OutputManager:
    """
    负责把 display() 的表、plt.show() 的图保存到当前目录。
    通过 set_dir()/set_tag() 控制输出位置与命名。
    """
...
def display(obj):
    """
    覆盖 notebook 的 display：既保存（如果在 capture_outputs 内），也显示/打印。
    """
...
def write_settings_txt(out_dir: Path, cfg: dict, base_dir: Path = None):
    """
    运行仍使用 cfg 内的绝对路径（由 normalize_config_row 生成，最稳）。
    但在 settings.txt 中，把 *_XLSX 记录为相对 base_dir 的相对路径，便于迁移/复现。
    """
...
def normalize_config_row(row: pd.Series, base_dir: Path) -> dict:
    ...
    for k in [
        ...
        "CLINICAL_BASE_FEATURES", "Y_CANDIDATES"
    ]:
    ...
    # RANDOM_STATE 统一使用默认值42，不再由训练文件指定
    cfg["RANDOM_STATE"] = DEFAULT_CONFIG["RANDOM_STATE"]
    # N_SPLITS 统一使用默认值5，不再由训练文件指定
    cfg["N_SPLITS"] = DEFAULT_CONFIG["N_SPLITS"]
    # 特征筛选参数统一使用默认值，不再由训练文件指定
    ...
    # Divide: 训练计划文件不指定时默认为 None（不进行随机分层划分）
...
# ============================================================
# 3) NEW: Train/Test split by CLOSED intervals with strict validation
# ============================================================

def _interval_overlap(a_min, a_max, b_min, b_max) -> bool:
    return not (a_max < b_min or b_max < a_min)

def split_train_test_by_closed_intervals(ids: pd.Index, cfg: dict, y=None):
    """
    划分规则：
    - 若 Internal_MIN/MAX 提供，则筛选样本ID范围
    - 若 Divide 参数提供（如 "5:3"），按比例分层随机划分训练集和验证集
    - 若 Divide 为空，则所有样本均作为训练集，验证集为空
    - Divide_Random 用于设置随机种子
    - y 用于分层划分，确保训练集和测试集的类别分布一致
    """
    ...
    # Filter ids within internal range if provided
    ...
    # If Divide is not provided, use all samples as training set (no split)
    ...
    # Stratified random shuffle and split
    # Use local RandomState to avoid affecting global seed
    rng = np.random.RandomState(divide_random)
    
    if y is not None:
        # 分层划分：确保训练集和测试集的类别分布一致
...
# ============================================================
# 4) NEW: Feature selection expression (parentheses AND/OR)
# ============================================================
...
def fs_spearman_correlation(X_modality: pd.DataFrame, y_tr: np.ndarray, threshold: float = 0.8, random_state: int = 42):
    """
    Spearman相关性筛选：对单个模态的特征进行筛选
    筛选出绝对值超过threshold的特征对/团，保留与结局变量单变量关联最强的特征
    
    参数：
        X_modality: 单个模态的特征DataFrame
        y_tr: 结局变量
        threshold: 相关性阈值，默认0.8
        random_state: 随机种子
    返回：
        list: 筛选后的特征名列表（按选择顺序排列）
    """
...
def run_feature_selection(X_tr: pd.DataFrame, y_tr: np.ndarray, cfg: dict):
    # 特征筛选始终执行（Per-Modality 模式：Spearman → mRMR → LASSO）
    ...
    # SUV modality: Spearman → mRMR → LASSO
    if suv_cols:
    ...
    # T2W modality: Spearman → mRMR → LASSO
    if t2w_cols:
    ...
    # ADC modality: Spearman → mRMR → LASSO
    if adc_cols:
    ...
    # Add clinical features (direct add, no selection)
...
# ============================================================
# 5) NEW: SMOTE (after feature selection)
# ============================================================

def _maybe_smote_xy(X_num: np.ndarray, y: np.ndarray, random_state: int):
    """
    安全 SMOTE：
    - 若少数类 < 2：无法 SMOTE，直接返回原数据
    - 动态设置 k_neighbors <= minority_count - 1（避免 n_neighbors 报错）
    - 任何异常：直接跳过 SMOTE（不让整轮模型崩）
    """
    try:
    ...
    try:
        ...
        k = min(5, minority - 1)  # SMOTE 需要 k_neighbors <= minority-1
...
def _impute_df(X_fit_df: pd.DataFrame, X_apply_df: pd.DataFrame):
    """
    用 X_fit_df 拟合 SimpleImputer，然后对 X_apply_df 做 transform。
    始终返回 DataFrame（保留 feature names），从根源消除 sklearn 警告。
    """
    imp = SimpleImputer(strategy="median")
    imp.fit(X_fit_df)
...
# ============================================================
# 6) NEW: scoring helper with optional SMOTE inside CV fold
# ============================================================
...
def oof_scores_with_optional_smote(estimator, X_df, y, cv, use_smote: bool, random_state: int,
    ...
    """
    OOF scores：
    - use_smote=True：在每个 fold 的训练子集上做 SMOTE（在 feature selection 之后）
      由于 estimator 通常含 imputer，我们这里对 X_tr/X_te 做同一 imputer，保证数值矩阵
    """
...
def build_linsvc_pipeline(C: float, random_state: int):
    """构建 Calib_LinearSVC 对应的基础 LinearSVC Pipeline。"""
    return Pipeline([
        ("imputer", SimpleImputer(strategy="median")),
        ("var", VarianceThreshold(0.0)),
        ("scaler", PerDatasetZScoreScaler()),
...
def _iter_grid(grid_dict):
    """将参数网格 dict 展开为所有参数组合的迭代器（按 grid_dict 顺序迭代）。"""
...
def _params_sort_key(model_name: str, params: dict):
    """为不同模型定义参数排序键（用于平局时选择更保守/更强正则化的组合）。"""
    if model_name in ("LogReg_ElasticNet", "Calib_LinearSVC", "SVM"):
        return params.get("C", 0)  # 较小 C -> 更强正则化，优先保留
...
def tune_model_params(model_name: str, builder, grid: dict, fallback_params: dict,
    ...
    参数：
        ...
        builder:    接收 params dict + random_state 返回 estimator 的函数
        grid:       参数网格 dict，如 {"C": [0.01, 0.1, 1.0]}
        fallback_params: 所有组合失败时的回退参数
        X_tr, y_tr: 训练数据
        cv, use_smote, random_state: CV 配置

    返回：
        best_params, best_auc, best_scores
    """
    combos = list(_iter_grid(grid))
    # 按平局偏好排序（让"更保守"的组合先出现，利用严格 > 实现 tie-break）
    ...
    for idx, params in enumerate(combos, start=1):
        try:
            ...
            # 严格 > 实现 tie-break：相同 AUC 时保留先出现的（更保守）组合
            if auc_val > best_auc:
                best_auc = auc_val
                best_params = params
                best_scores = scores
...
def nested_cv_tune_and_score(model_name: str, builder, grid: dict, fallback_params: dict,
                              ...
                              fs_cfg: dict = None):
    """
    嵌套CV实现：使用内层CV在每个外层fold上独立调优超参数，
    从而得到无偏的OOF AUC估计。
    ...
    参数：
        ...
        fallback_params: 所有组合失败时的回退参数
        X_df, y: 训练数据
        outer_cv: 外层CV划分（通常5折）
        inner_cv_folds: 内层CV折数（通常3折）
        use_smote: 是否使用SMOTE
        random_state: 随机种子

    返回：
        oof_scores: 无偏的OOF预测概率数组
        best_params_global: 在完整训练集上找到的最佳参数
        best_auc_global: 完整训练集上的最佳AUC（内层CV平均）
    """
    n_samples = len(y)
    oof_scores = np.zeros(n_samples, dtype=float)
    oof_scores[:] = np.nan

    # 收集每个外层fold找到的最佳参数，用于最终在完整训练集上训练
    fold_best_params = []
    ...
    # 使用完整训练集进行最终的CV调参，找到全局最佳参数（使用5折CV和全局种子）
...
def load_model_from_file(model_name: str, task_name: str, exp_dir, suffix: str = "final"):
    """
    加载已保存的模型
    ...
    返回：
        加载的模型对象，如果文件不存在则返回None
    """
    task_s = _sanitize_filename(task_name)
    model_s = _sanitize_filename(model_name)
    model_path = exp_dir / f"task_{task_s}" / "models" / f"{model_s}_{suffix}.joblib"
...
def load_best_params(model_name: str, task_name: str, exp_dir):
    """
    加载已保存的最佳参数
    ...
    返回：
        最佳参数字典，如果文件不存在则返回None
    """
    task_s = _sanitize_filename(task_name)
    model_s = _sanitize_filename(model_name)
    params_path = exp_dir / f"task_{task_s}" / "models" / f"{model_s}_best_params.json"
    
    if params_path.exists():
        with open(params_path, 'r') as f:
            params = json.load(f)
        print(f"[LOAD] Best params loaded: {params_path}")
        return params
    else:
        print(f"[LOAD] Best params file not found: {params_path}")
        return None


# 需要调参的模型配置：(builder, grid, fallback_params)
TUNE_CONFIGS = {
    "LogReg_ElasticNet": (
...
def tune_elasticnet_params(X_tr: pd.DataFrame, y_tr: np.ndarray, cv, use_smote: bool, random_state: int):
    """
    保留的兼容包装器，内部调用通用 tune_model_params。
    """
    builder, grid, fallback = TUNE_CONFIGS["LogReg_ElasticNet"]
    return tune_model_params("LogReg_ElasticNet", builder, grid, fallback,
                              X_tr, y_tr, cv, use_smote, random_state)


def bootstrap_auc_ci(y_true, y_score, n_bootstrap=1000, alpha=0.05, random_state=None):
    """
    使用Bootstrap法计算AUC的置信区间和标准差
...
def bootstrap_pr_auc_ci(y_true, y_score, n_bootstrap=1000, alpha=0.05, random_state=None):
    ...
    返回：
    - pr_auc_mean: PR-AUC均值
    - pr_auc_ci: (lower, upper) 置信区间
    - pr_auc_std: PR-AUC标准差
    """
    rng = np.random.RandomState(random_state)
    n_samples = len(y_true)
    pr_auc_values = []
...
def compute_youden_index(y_true, y_score):
    ...
    返回：
    - dict: 包含最优阈值、约登指数、灵敏度、特异度、PPV、NPV
    """
...
# ============================================================
# 7) NEW: SHAP for one (model, dataset)
# ============================================================
...
def _xgboost_pred_contribs_shap_values(clf, X_trans):
    ...
    if contribs.ndim == 3:
        # XGBoost has used both (n, classes, features+1) and
        # (n, features+1, classes) layouts across versions/settings.
...
def plot_shap_for_estimator(model_name: str, estimator, X_df: pd.DataFrame, dataset_tag: str, task_name: str):
    """
    输出 2 张图：bar + beeswarm
    - dataset_tag: "train" / "test"
    """
...
# ============================================================
# 7a) Prediction Probability Distribution Histogram
# ============================================================
...
def plot_prediction_histogram(model_name: str, task_name: str,
    ...
    """
    绘制单个模型在训练集(OOF)、验证集、外部验证集上的预测概率分布直方图
    
    参数：
    - model_name: 模型名称
    - task_name: 任务名称
    - y_true_train, y_score_train: 训练集真实标签和OOF预测概率
    - y_true_test, y_score_test: 验证集真实标签和预测概率（可选）
    - y_true_ext, y_score_ext: 外部验证集真实标签和预测概率（可选）
    """
    datasets = []
    ...
    for idx, (ds_name, y_true, y_score) in enumerate(datasets):
        ...
        if score_min < -0.1 or score_max > 1.1:
            # 不是概率，可能是decision_function输出，尝试归一化
...
# ============================================================
# 7b) Decision Curve Analysis (DCA) - Combined Multi-Model
# ============================================================
...
def plot_combined_dca(dataset_tag: str, task_name: str, 
    ...
    参数：
    - dataset_tag: "train" / "test" / "external"
    - task_name: 任务名称
    - model_preds: [(model_name, y_true, y_score), ...]
    """
    try:
        if not model_preds:
        ...
        # 计算所有模型的净获益曲线
        all_thresholds = np.linspace(0, 1.0, 201)  # 包含0
        model_curves = []
        ...
        for model_name, y_true, y_score in model_preds:
            ...
            for pt in all_thresholds:
                if pt >= 1.0:
                    ...
                    # Treat-all不应为负（标准DCA约定：超过此阈值则Treat-All不如Treat-None）
                    nb_all = max(0, nb_all)
        ...
        # 计算全局ylim（取所有模型和Treat-All的最大净获益）
        all_nb = []
        ...
        # Treat-All（使用第一个模型的Treat-All，因为患病率相同）
...
# ============================================================
# 7c) Calibration Curve - Combined Multi-Model
# ============================================================
...
def run_one_task_all_models(task_name: str, y_dict: dict, X_all: pd.DataFrame, X_ext: pd.DataFrame, out_df: pd.DataFrame, cfg: dict, out_mgr: OutputManager):
    ...
    # Filter models based on ALGORITHMS config
    ...
    # 1) Train OOF ROC: plot all models on one figure (train)
    plt.figure(figsize=(7, 6))
    train_auc_rows = []
    test_auc_rows = []
    test_roc_curves = []   # list of (model_name, fpr, tpr, auc)
    external_auc_rows = []
    external_roc_curves = []
    
    # PR-AUC tracking
    train_pr_auc_rows = []
    train_pr_curves = []   # list of (model_name, precision, recall, pr_auc)
    test_pr_auc_rows = []
    test_pr_curves = []   # list of (model_name, precision, recall, pr_auc)
    external_pr_auc_rows = []
    external_pr_curves = []
    
    # Youden Index tracking
    train_youden_rows = []
    test_youden_rows = []
    external_youden_rows = []
    
    # 存储各模型的预测概率，供后续DCA/校准/直方图使用
    oof_scores_dict = {}      # model_name -> OOF训练集预测概率
    test_scores_dict = {}     # model_name -> 验证集预测概率
    ext_scores_dict = {}      # model_name -> 外部验证集预测概率
    tuned_models = {}         # model_name -> 调参后的estimator（供SHAP/DCA/校准循环使用）
    
    # 准备外部验证集数据（使用训练集的统计数据进行z-score标准化）
        ...
        try:
            if name in TUNE_CONFIGS:
                ...
                if name == "XGBoost" and not XGBOOST_AVAILABLE:
                    ...
                    # 使用嵌套CV寻找无偏的OOF分数和最佳参数
                    # 模型训练阶段使用独立的随机种子（全局种子+1314），与数据划分/评估阶段隔离
                    ...
                    # 用最佳参数重建 estimator，供后续 test/external 在完整训练集上重新训练最终模型
            ...
            # 存储调参后的estimator，供后续SHAP/DCA/校准循环使用
            ...
            # test AUC: fit on full train (optionally SMOTE), then eval on test
            s_te = np.array([])
            est2 = None
            X_tr_imp = None
            ...
            if split_enabled and X_te is not None and len(X_te) > 0:
                ...
                if use_smote:
                    # 统一用 DataFrame，避免 feature-name warning
                ...
                # === Youden Index on Test ===
                try:
                    yi_te = compute_youden_index(y_te, s_te)
            ...
            if external_enabled and X_ext_selected is not None and len(X_ext_selected) > 0 and y_ext is not None:
                ...
                try:
                    ...
                    # 预测外部验证集（PerDatasetZScoreScaler 会自动使用外部验证集的统计量进行标准化）
                    ...
                    # === Youden Index on External ===
                    try:
                        yi_ext = compute_youden_index(y_ext, s_ext)
    ...
    # External PR-AUC table and plot
    ...
    # 2) SHAP + DCA + Calibration + Histogram: 对所有模型都输出分析
    if tuned_models is not None and len(tuned_models) > 0:
        # 收集各数据集的预测结果，用于绘制合并的DCA和校准曲线
        dca_train_data = []    # [(model_name, y_true, y_score), ...]
        dca_test_data = []
        dca_external_data = []
        ...
        for name, est in tuned_models.items():
            ...
            try:
                ...
                # === 使用存储的OOF/验证/外部预测概率 ===
                # 训练集：使用OOF预测概率（避免数据泄露）
                s_tr_oof = oof_scores_dict.get(name)
                
                # 验证集和外部验证集：使用完整训练后的预测概率
    ...
    # return summaries (for experiment-level summary)
    # “最优模型”仍给出，但不影响 test（test 已对所有模型跑过）
    best_by_train = None
    best_auc = None
...
def run_one_experiment(cfg: dict, out_dir: Path) -> pd.DataFrame:
    ...
    try:
        with capture_outputs(out_mgr):
            ...
            y_dict = add_task_pda_idcp(y_dict)
            ...
            for task in tasks:
                ...
                # IMPORTANT: interval errors should stop this experiment row immediately (as you要求)
                ret = run_one_task_all_models(task, y_dict, X_all, X_ext, out_df, cfg, out_mgr)
                if ret is None:
                    continue

                # 记录训练集和验证集的ID（取第一个任务的划分）
            ...
            # Summary table for this experiment (仅作为返回值供 main 汇总，不再单独保存文件)
...
def save_model_predictions(
    ...
    参数：
        ...
        train_y: 训练集标签
        test_y: 测试集标签
        external_probs: 外部验证集预测概率（可选）
        external_y: 外部验证集标签（可选）
        train_outcomes/test_outcomes/external_outcomes: 对应样本在 Outcome 文件中的完整指标
    """
    task_s = _sanitize_filename(task_name)
